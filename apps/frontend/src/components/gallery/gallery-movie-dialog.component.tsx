...
export const GalleryMoviesDialog: React.FC<GalleryMoviesDialogProps> = ({
  ...
  const defaultValuesTypeAdd = {
    title: "",
    duration: "",
    plot: "",
  };

  const defaultValuesTypeEdit = {
    title: movie?.title,
    genre: movie?.genre,
    duration: movie?.duration,
    plot: movie?.plot,
    release_date: movie?.release_date as unknown as Date,
    poster: movie?.poster,
  };

  const form = useForm<GalleryMovieDialogValues>({
    resolver: zodResolver(GalleryMovieDialogSchema),
    defaultValues: defaultValuesTypeAdd,
  });

  // Update form values when movie prop changes
  useEffect(() => {
    if (!isTypeAdd && movie) {
      form.reset(defaultValuesTypeEdit);
    }
